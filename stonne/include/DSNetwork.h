...
private:
  ...
  int nlevels; // Number of levels of the DS without taking into account the MS
               // level
  std::map<std::pair<int, int>, DSwitch*>
    dswitchtable; // Map with the switches of the topology. The connection among
                  // them will be different depending on the topology used
  std::map<std::pair<int, int>, Connection*>
    connectiontable;           // Outputs connections of each level.
...
public:
  DSNetwork(id_t id,
            ...
            Connection* inputConnection); // ms_size = ms_size of the group that
                                          // contain this tree
  ~DSNetwork();
  ...
  void printStats(std::ofstream& out,
                  unsigned int indent); // Print the stats of the component
