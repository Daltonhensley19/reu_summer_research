...
private:
  ...
  std::map<std::pair<int, int>, Connection*>
    verticalconnectiontable; // Table with the vertical connections
  std::map<std::pair<int, int>, Connection*>
    horizontalconnectiontable; // Table with the horizontal connections
  std::map<std::pair<int, int>, Connection*>
    accbufferconnectiontable; // Table with the accbuff connections
  unsigned int ms_rows;       // Number of rows in the ms array
  unsigned int ms_cols;       // Number of columns in the ms array
  unsigned int
    forwarding_ports; // MSNetwork needs this parameter to create the network
  unsigned int buffers_capacity; // Capacity of the buffers in the MSwitches.
                                 // This is neccesary to check if it is feasible
                                 // to manage the folding.
  ...
  // std::map<std::pair<int,int>, Connection*> getTopConnections(); //Return the
  // connections

public:
  /*
     By the default the implementation of the MS just receives a single element,
     calculate a single psum and/or send a single input activation to the
     neighbour. This way, the parameters input_ports, output_ports and
     forwarding_ports will be set as the single data size. If this
     implementation change for future tests, this can be change easily bu
     mofifying these three parameters.
   */
