...
public:
  ...
  unsigned int output_ports; // Number of output ports. By default this will be
                             // 2
...
{
public:
  cycles_t latency; // Latency of the MS to perform a multiplication. This
                    // number is expressed in number of cycles. //TODO To imple
  unsigned int input_ports; // Number of input ports of the MS. This number is 1
                            // by default in MAERI
  unsigned int output_ports; // Number of output ports of the MS.
  unsigned int
    forwarding_ports; // Number of forwarding ports of the MS. This is basically
                      // the number of elements that can be forwarded in a
                      // single cycle and in MAERI architecture is just 1.
  unsigned int port_width;       // Bit width
  unsigned int buffers_capacity; // Number of elements that can be stored in the
                                 // MS buffers. TODO In future implementations
                                 // this could be splited up, taking each buffer
                                 // capacity in a different parameter.
...
public:
  ReduceNetwork_t
    reduce_network_type; // Type of the ReduceNetwork configured in this moment
...
public:
  ...
  unsigned int input_ports; // Number of input ports of the ASwitch. By  default
                            // in MAERI this is just 2
  unsigned int output_ports; // Number of output ports of the ASwitch. By
                             // default  in MAERI this is 1
  unsigned int forwarding_ports; // Nuber of forwarding ports of the ASwitch.
  unsigned int port_width;       // Bit width
  cycles_t latency; // Latency of the AS to perform. This number is expressed in
                    // number of cycles. //TODO To implement
...
{
public:
  cycles_t latency; // Latency of the LookUpTable to perform. This number must
                    // be expressed in number of cycles. 0 no supported
...
{
public:
  MemoryController_t mem_controller_type; // Memory controller type (e.g.,
                                          // DENSE_WORKLOAD or SPARSE_GEMM)
  unsigned int write_buffer_capacity; // Capacity of the buffers expressed in
                                      // terms of number of elements
  unsigned int n_read_ports;  // dn_bw
  unsigned int n_write_ports; // rn_bw
  unsigned int port_width;    // Bit width
  unsigned int
    n_multiplier_configurations; // Number of multiplier configurations
  unsigned int
    n_reduce_network_configurations; // Number of reduce network configurations
...
public:
  // General parameters
  unsigned int
    print_stats_enabled; // Specified whether the statistics must be printed.
  ...
  // Indicates whether according to the hardware parameters, sparsity is enabled
  // in the architecture
  bool sparsitySupportEnabled();

  // Indicates whether according to the hardware parameters, the operation of
  // CONV itself is supported. Otherwise, the operation can be done using GEMM
  // operation
