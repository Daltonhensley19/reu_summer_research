...
public:
  ...
  unsigned int
    base_addr; // Base addr of this VN (i.e., the first element to compute).
...
private:
  ...
  ReduceNetwork*
    reduce_network; // This is not used in this controller as the configuration
                    // is performed in STONNEModel when the tile is loaded, and
                    // this is needed just once
  MultiplierNetwork* multiplier_network; // Idem as reduce_network
  Connection* write_connection;
  // Connection* read_connection;
  std::vector<Connection*>
    read_connections; // Input port connections. There are as many connections
                      // as n_read_ports are specified.
  ...
  std::vector<Fifo*>
    input_fifos; // Fifos used to store the inputs before being fetched
  std::vector<Fifo*>
    psum_fifos; // Fifos used to store partial psums before being fetched
  ...
  // Signals
  bool weights_distributed; // Indicates if the weights have been distributed
                            // for a certain iteration
  bool fw_link_enabled; // Indicates if the fw link is enabled in this cycle and
                        // therefore the number of bw used per cycle is less
  bool weights_finished; // Flag that indicates that all the weights have been
                         // delivered
  bool input_finished; // Flag that indicates that all the inputs have been
                       // delivered
  bool tile_loaded; // SPecify if the tile is loaded
  bool
    execution_finished; // Flag that indicates when the execution is over. This
                        // happens when all the opixels have been calculated.
  ...
  unsigned int
    current_output_pixel; // This variable has the count for the current number
                          // of output pixels calculated
  unsigned int
    output_pixels_to_compute; // This variable has the number of output pixels
                              // that the simulator must calculate before
  ...
  std::list<DataPackage*>
    packages_created; // Vector used to track the packages and delete them at
                      // the end of the execution
  std::vector<Connection*> write_port_connections;
  VNAT_Register**
    VNAT; // VNAT with as many registers as VN configured in the accelerator
  ...
  void send();
  std::vector<Connection*> getWritePortConnections() const
  {
    return this->write_port_connections;
  }
...
public:
  SDMemory(id_t id,
           ...
           Connection* write_connection);
  ~SDMemory();
  void setLayer(DNNLayer* dnn_layer,
                address_t input_address,
                address_t filter_address,
                address_t output_address,
                Dataflow dataflow);
  ...
  void setWriteConnections(
    std::vector<Connection*>
      write_port_connections); // All the write connections must be set at a
                               // time
  void setSparseMetadata(metadata_address_t MK_metadata,
                         metadata_address_t KN_metadata,
                         metadata_address_t output_metadata)
  {
    assert(false);
  } // Not supported by this controller
  void setReduceNetwork(ReduceNetwork* reduce_network)
  {
    this->reduce_network = reduce_network;
  }
  // Used to configure the MultiplierNetwork according to the controller if
  // needed
  void setMultiplierNetwork(MultiplierNetwork* multiplier_network)
  {
    this->multiplier_network = multiplier_network;
  }
