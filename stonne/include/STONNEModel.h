...
private:
  ...
  MemoryController*
    mem; // MemoryController abstraction (e.g., SDMemory from MAERI)
  Bus* collectionBus;             // CollectionBus
  Connection* outputASConnection; // The last connection of the AS and input to
                                  // the lookuptable
  Connection* outputLTConnection; // Output of the lookup table connection and
                                  // write port to the SDMemory
  Connection** addersBusConnections; // Array of output connections between the
                                     // adders and the bus
  Connection** BusMemoryConnections; // Array of output Connections between the
                                     // bus and the memory. (Write output ports)
  ...
  void connectMSNandDSN(); // Function to connect the multiplieers of the MSN to
                           // the last level switches in the DSN.
...
public:
  ...
  void loadDNNLayer(Layer_t layer_type,
  ...
  // Load CONV Layer. At the end this calls to the general constructor  with all
  // the parameters
  void loadCONVLayer(std::string layer_name,
                     unsigned int R,
                     unsigned int S,
                     unsigned int C,
                     unsigned int K,
  ...
  // N = batch size (i.e., number of rows in input matrix); S=number of inputs
  // per batch (i.e., column size in input matrix and weight matrix); K=number
  // of outputs neurons (i.e, number of rows weight matrix)
  void loadFCLayer(std::string layer_name,
                   unsigned int N,
                   unsigned int S,
                   unsigned int K,
                   address_t input_address,
  ...
  // Load Dense GEMM onto STONNE according to SIGMA parameter taxonomy and
  // tiling according to T_N, T_K and T_M
  void loadDenseGEMM(std::string layer_name,
                     unsigned int N,
                     unsigned int K,
                     unsigned int M,
                     address_t MK_matrix,
  ...
  void loadSparseDense(std::string layer_name,
                       unsigned int N,
                       unsigned int K,
                       unsigned int M,
                       address_t MK_matrix,
  ...
  void loadTile(unsigned int T_R,
                unsigned int T_S,
                unsigned int T_C,
                unsigned int T_K,
                unsigned int T_G,
                unsigned int T_N,
                unsigned int T_X_,
                unsigned int T_Y_); // Load general and CONV tile
  void loadFCTile(unsigned int T_S,
                  unsigned int T_N,
                  unsigned int T_K); // VNSize = T_S, NumVNs= T_N*T_K
  void run();
};
