...
private:
  ...
  MultiplierNetwork*
    multiplier_network; // Multiplier network used to be reconfigured

  unsigned int M;
  unsigned int N;

  unsigned int K; // Number of columns MK matrix and rows KN matrix. Extracted
                  // from dnn_layer->get_C();
  ...
  OSMeshControllerState
    current_state; // Stage to control what to do according to the state
  std::vector<SparseVN>
    configurationVNs; // A set of each VN size mapped onto the architecture.
  std::vector<unsigned int> vnat_table; // Every element is a VN, indicating the
                                        // column that is calculating
  ...
  unsigned int
    T_K; // This is the actual value of tile of K. This is just 1 in this case
  unsigned int T_M;
  unsigned int iter_N;
  unsigned int iter_K;
  unsigned int iter_M;
  ...
  // Signals
  bool configuration_done; // Indicates whether the architecture has been
                           // configured to perform the delivering
  bool execution_finished; // Flag that indicates when the execution is over.
                           // This happens when all the output values have been
                           // calculated.
  bool iteration_completed;

  bool
    metadata_loaded; // Flag that indicates whether the metadata has been loaded
...
public:
  ...
  void setSparseMatrixMetadata(metadata_address_t MK_metadata_id,
