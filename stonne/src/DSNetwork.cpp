...
// This Constructor creates the distribution tree similar to the one shown in
// the paper
DSNetwork::DSNetwork(id_t id,
                     std::string name,
                     Config stonne_cfg,
  ...
  : Unit(id, name)
{
  ...
  assert(ispowerof2(ms_size)); // Ensure the number of multipliers is power
                               // of 2.

  this->inputConnection = inputConnection;
  int nlevels           = log10(ms_size) /
                log10(2); // All the levels without count the leaves (MSwitches)
  ...
  { // From root to leaves (without the MSs)
    ...
    { // From left to right of the structure
      ...
      if (i == 0)
      { // The first node is connected to the inputConnection. This is an
        // extreme case but it is more clear to do it inside loop
        ds->setInputConnection(inputConnection);
      }

      else
      {
        std::pair<int, int> sourcepair(
          i - 1, j); // the same number of connections in the above level
                     // matches the number of Dswitches in this l
      ...
      for (int c = 0; c < CONNECTIONS_PER_SWITCH; c++)
      {
        Connection* connection =
          new Connection(port_width); // Output link so output ports
        int connection_pos_this_level =
          j * CONNECTIONS_PER_SWITCH +
          c; // number of switches alreay created + shift this switch
...
DSNetwork::~DSNetwork()
{
  ...
  for (std::map<std::pair<int, int>, DSwitch*>::iterator it =
         dswitchtable.begin();
       it != dswitchtable.end();
       ++it)
  {
  ...
  for (std::map<std::pair<int, int>, Connection*>::iterator it =
         connectiontable.begin();
...
std::map<int, Connection*> DSNetwork::getLastLevelConnections()
{
  int last_level_index =
    this->nlevels -
    1; // The levels start from 0, so in the table the last level is nlevels-1
  std::map<int, Connection*>
    connectionsLastLevel; // Map with the connections of the last level of DS
  for (int i = 0; i < this->ms_size; i++)
  { // Each multiplier must have its own connection if the DSNetwork has been
    // created correctly
...
{
  // Iterate over each DS from the root to the leaves and from left to right.
  // Each DSwitch will execute its cycle and will receive data from the top
  // and will send it to the lower level by using source routing.

  // The order from root to leaves is important in terms of the correctness of
  // the network.
...
{
  out << ind(indent) << "{"
      << std::endl; // Since it is inside an array we do not name the object
  // out << ind(indent+IND_SIZE) << "\"ms_size\" : " << this->ms_size  <<
  // std::endl; DSNetwork global parameters
  out << ind(indent + IND_SIZE) << "\"DSwitchStats\" : ["
  ...
  { // From root to leaves (without the MSs)
    ...
    { // From left to right of the structure
      ...
      if (j == (switches_this_level - 1))
      { // If I am in the last switch of the level, the comma to separate the
        // swes is not added
        out << std::endl; // This is added because the call to ds print do not
                          // show it (to be able to put the comma, if neccesary)
      }
      else
      {
        out << ","
            << std::endl; // Comma and line break are added to separate with the
                          // next ASwitch in the array of this level
      }
    }
    if (i == (this->nlevels - 1))
    { // If I am in the last level, the comma to separate the different levels
      // is not added
      out << ind(indent + IND_SIZE + IND_SIZE) << "]" << std::endl;
    }

    else
    { // If I am not in the last level, then the comma is printed to separate
      // with the next level
      out << ind(indent + IND_SIZE + IND_SIZE) << "]," << std::endl;
    }
...
void DSNetwork::printEnergy(std::ofstream& out, unsigned int indent)
{
  /*
       This component prints:
           - wires that connect every DSwitch with its child (include the wires
     that connect with the MSNetwork)
           - DSwitches
  */
