...
OSMeshSDMemory::OSMeshSDMemory(id_t id,
                               std::string name,
                               Config stonne_cfg,
  ...
  : MemoryController(id, name)
{
  ...
  this->ms_size_per_input_port =
    (this->ms_rows + this->ms_cols) / this->n_read_ports;
...
void OSMeshSDMemory::setWriteConnections(
...
void OSMeshSDMemory::setReadConnections(
...
void OSMeshSDMemory::setLayer(DNNLayer* dnn_layer,
                              ...
                              Dataflow dataflow)
{
  ...
  // assert(this->dnn_layer->get_layer_type()==DenseGEMM);  // This controller
  // only supports GEMM with one sparse and one dense this->dataflow = dataflow;
  ...
  this->N =
    this->dnn_layer->get_K(); // In this case both parameters match each other.
...
void OSMeshSDMemory::setTile(Tile* current_tile)
{
  this->T_M =
    current_tile
      ->get_T_X_(); // According to loadGemmTile function in STONNE the T_M gemm
                    // parameter is saved in T_K DNN parameter
...
void OSMeshSDMemory::cycle()
{
  ...
  { // Initialize these for the first time
    ...
    this->cols_used =
      (remaining_N < T_N) ? remaining_N : T_N; // max(remaining_N, T_N)
    ...
    this->multiplier_network->configureSignals(
      tile1, this->dnn_layer, this->ms_rows, this->ms_cols);
    this->reduce_network->configureSignals(
      tile1, this->dnn_layer, this->ms_rows * this->ms_cols, this->iter_K);
  ...
  if (current_state == OS_DIST_INPUTS)
  {
    ...
    for (int i = 0; i < this->cols_used; i++)
    {
      ...
      data =
        this->KN_address[(index_N + i) * this->K +
                         this->current_K]; // Notice that in dense operation the
                                           // KN matrix is actually NK
      sdmemoryStats.n_SRAM_weight_reads++;
      DataPackage* pck_to_send =
        new DataPackage(sizeof(data_t), data, WEIGHT, 0, UNICAST, i);
      this->sendPackageToInputFifos(pck_to_send);
    }

    // SENDING M PACKAGES. Note that not delay is necessary as the fifos will
    // carry out these delays
    for (int i = 0; i < rows_used; i++)
    {
      ...
      DataPackage* pck_to_send = new DataPackage(
        sizeof(data_t), data, IACTIVATION, 0, UNICAST, i + this->ms_cols);
      this->sendPackageToInputFifos(pck_to_send);
    }
  ...
  if (!write_fifo->isEmpty())
  {
    ...
    for (int i = 0; i < write_fifo->size(); i++)
    {
      ...
      unsigned int current_tile_M_pointer =
        (n_iterations_completed / this->iter_N) *
        this->T_M; // Change this to change the dataflow
      unsigned int current_tile_N_pointer =
        (n_iterations_completed % this->iter_N) * T_N;
      unsigned int vn_M_pointer = vn / this->cols_used;
      unsigned int vn_N_pointer = vn % this->cols_used;
      unsigned int addr_offset =
        (current_tile_M_pointer + vn_M_pointer) * this->N +
        current_tile_N_pointer + vn_N_pointer;
      vnat_table[vn]++;
      ...
      {
        std::cout << "Output completed " << current_output << "/" << M * N
                  << ")" << std::endl;
      }
  ...
  else if (current_state == OS_DIST_INPUTS)
  {
    if (iteration_completed)
    {
      ...
      { // Change the to change the dataflow
