...
void save_cache_data_load_sparse(int data_index)
{
...
void save_cache_data_store_sparse(int data_index, int data)
{
...
void save_raw_data(data_t data, int data_index)
{
...
SparseSDMemory::SparseSDMemory(id_t id,
                               std::string name,
                               Config stonne_cfg,
  ...
  : MemoryController(id, name)
{
  ...
  this->output_size            = 0;
  this->sta_current_index_metadata =
    0; // Stationary matrix current index (e.g., row in MK)
...
void SparseSDMemory::setWriteConnections(
...
void SparseSDMemory::setReadConnections(
...
void SparseSDMemory::setLayer(DNNLayer* dnn_layer,
                              ...
                              Dataflow dataflow)
{
  ...
  assert(this->dnn_layer->get_layer_type() ==
         GEMM); // This controller only supports GEMM with sparsity
  this->dataflow = dataflow;

  this->output_address = output_address;
  this->layer_loaded   = true;

  // Loading parameters according to the equivalence between CNN layer and GEMM.
  // This is done in this way to keep the same interface.
  this->M = this->dnn_layer->get_K();
  this->K = this->dnn_layer
              ->get_S(); // Be careful. K in GEMMs (SIGMA taxonomy) is not the
                         // same as K in CNN taxonomy (number of filters)
  this->N =
    this->dnn_layer->get_X(); // In this case both parameters match each other.
...
// Load bitmaps
void SparseSDMemory::setSparseMetadata(metadata_address_t MK_metadata,
                                       metadata_address_t KN_metadata,
...
void SparseSDMemory::cycle()
{
  ...
  { // If the architecture has not been configured
    ...
    {         // We are managing one cluster with folding
      n_ms++; // One for the psum
      j = this->sta_current_j_metadata;
      while ((n_ms < this->num_ms) && (j < K))
      { // TODO change MK if it is another dw
        if (this->STA_metadata[i * STA_DIST_VECTOR + j * STA_DIST_ELEM])
        { // If the bit is enabled in the stationary bitmap
          // Add to the cluster
          this->sta_counters_table[i * STA_DIST_VECTOR + j * STA_DIST_ELEM] =
            n_ms; // DEST
          n_ms++;
          n_current_cluster++;
        }
      ...
      SparseVN VN(
        n_current_cluster,
        true); // Here folding is enabled and the SparseVN increments 1 to size
      this->configurationVNs.push_back(VN);
      this->vnat_table.push_back(
        0); // Adding the current calculation (row or column) of this VN.
      // Searching if there are more values in the next cluster. If not, update
      // sta_last_j_metadata to K to indicate that in the next iteration the
      // next sta dim must be evaluated
    ...
    { // Whole rows
      while ((n_ms < this->num_ms) && (i * K + j < dim_sta * K))
      { // TODO change MK if it is another dw
        if (this->STA_metadata[i * STA_DIST_VECTOR + j * STA_DIST_ELEM])
        { // If the bit is enabled in the stationary bitmap
          // Add to the cluster
      ...
      { // If any entire cluster fits, then folding is needed to manage this
        // cluster
        /*
     if((K-j) < 3) { //The next cluster must have cluster size greater or equal
     than 3 int n_elements_to_make_cluster_3 = 3-(K-j);
         j-=n_elements_to_make_cluster_3;
         n_current_cluster-=n_elements_to_make_cluster_3;
     }
     */
        SparseVN VN(n_current_cluster, false); // Here folding is still disabled
                                               // as this is the first iteration
        this->configurationVNs.push_back(VN);
        this->vnat_table.push_back(
          0); // Adding the current calculation (row or column) of this VN.
              // Searching if there are more values in the next cluster. If not,
              // update sta_last_j_metadata to K to indicate that in the next
              // iteration the next sta dim must be evaluated
        int remaining_values = 0;
        for (int r = j; r < K; r++)
        {
          if (this->STA_metadata[i * STA_DIST_VECTOR + r * STA_DIST_ELEM])
      ...
      { // If there is at least one cluster, then all of them has size K and it
        // is necessary to stream K
        // K elements
    ...
    for (int i = 0; i < dim_str; i++)
    {
      for (int j = 0; j < K; j++)
      {
        ...
        {
          // std::cout << "Value (" << i << ", " << j << "): " << source_id <<
          // std::endl;
          this->str_counters_table[i * STR_DIST_VECTOR + j * STR_DIST_ELEM] =
            source_id;
          source_id++;
        }
      }
    ...
    if (this->configurationVNs.size() == 0)
    {
      std::cout << "Cluster size exceeds the number of multipliers in row "
                << this->sta_current_index_metadata << std::endl;
      assert(false);
    }
    for (int i = 0; i < this->configurationVNs.size(); i++)
    {
      std::cout << "Found a VN of size "
                << this->configurationVNs[i].get_VN_Size() << std::endl;
    }
    this->sdmemoryStats.n_sta_vectors_at_once_avg +=
      this->configurationVNs.size(); // accumul
    if (this->configurationVNs.size() >
        this->sdmemoryStats.n_sta_vectors_at_once_max)
    {
      this->sdmemoryStats.n_sta_vectors_at_once_max =
        this->configurationVNs.size();
    }
    ...
    this->multiplier_network->configureSparseSignals(
      this->configurationVNs, this->dnn_layer, this->num_ms);
    // Configuring the reduce network
    this->reduce_network->resetSignals(); // Reseting the values to default
    this->reduce_network->configureSparseSignals(
      this->configurationVNs, this->dnn_layer, this->num_ms);
  ...
  else if (current_state == DIST_STA_MATRIX)
  {
    ...
    for (int i = 0; i < this->configurationVNs.size(); i++)
    {
      ...
      for (; j < this->configurationVNs[i].get_VN_Size(); j++)
      {
        // Accessing to memory
        data_t data =
          this->STA_address[sta_current_index_matrix +
                            sub_address]; // In both dataflows adjacents
                                          // elements are consecutive in mem
                                          //
                                          //

        // std::cout << "Data index: " << (sta_current_index_matrix +
        // sub_address)
        //<< std::endl;

        // std::cout << "Data: " << data << std::endl;

        auto data_index = sta_current_index_matrix + sub_address;
        save_raw_data(data, data_index); //keep
        save_cache_data_load_sparse(data_index);

        sdmemoryStats.n_SRAM_weight_reads++;
        this->n_ones_sta_matrix++;
        DataPackage* pck_to_send =
          new DataPackage(sizeof(data_t), data, WEIGHT, 0, UNICAST, dest);
  ...
  else if (current_state == DIST_STR_MATRIX)
  {
    ...
    if (this->sta_current_j_metadata > 0)
    { // If folding is enabled there is just a row on  fly
      assert(this->configurationVNs.size() == 1);
      // send psum
      unsigned int addr_offset = (sta_current_index_metadata)*OUT_DIST_VN +
                                 str_current_index * OUT_DIST_VN_ITERATION;
      ...
      data_t psum =
        this->output_address[addr_offset]; // Reading the current psum
        
      save_cache_data_load_sparse(addr_offset);
      save_raw_data(psum, addr_offset);
      // std::cout << "Data index: " << addr_offset << std::endl;
      // std::cout << "Data: " << psum << std::endl;

      DataPackage* pck = new DataPackage(
        sizeof(data_t), psum, PSUM, 0, MULTICAST, destinations, this->num_ms);
      this->sdmemoryStats.n_SRAM_psum_reads++; // To track information
      this->sendPackageToInputFifos(pck);
    }
    for (int j = init_point_str; j < end_point_str; j++)
    { // For each element in the current vector in the str matrix
      ...
      unsigned int last_sta_vector =
        sta_current_index_metadata + this->configurationVNs.size();
      for (int i = first_sta_vector; i < last_sta_vector; i++)
      { // For each vector in the sta matrix could exist a value to send the str
        // value
        if (STA_metadata[i * STA_DIST_VECTOR + j * STA_DIST_ELEM])
        { // ADD Destination
          unsigned int dest =
            sta_counters_table[i * STA_DIST_VECTOR + j * STA_DIST_ELEM];
      ...
      {
        unsigned int src =
          str_counters_table[str_current_index * STR_DIST_VECTOR +
                             j * STR_DIST_ELEM];
        data = STR_address[src];

        // TODO: Look into using a LRU or LFU cache to avoid costly reads to

        // std::cout << "Data Index: " << src << std::endl;
        // std::cout << "Data: " << data << std::endl;
        save_cache_data_load_sparse(src);
        sdmemoryStats.n_SRAM_input_reads++;

        // std::cout << "Input read: " << sdmemoryStats.n_SRAM_input_reads
        //<< std::endl;
      }

      else
      {
        data = 0.0; // If the STA matrix has a value then the STR matrix must be
                    // sent even if the value is 0
      }
  ...
  if (!write_fifo->isEmpty())
  {
    ...
    for (int i = 0; i < write_fifo->size(); i++)
    {
      ...
      unsigned int addr_offset =
        (sta_current_index_metadata + vn) * OUT_DIST_VN +
        vnat_table[vn] * OUT_DIST_VN_ITERATION;
      vnat_table[vn]++;
      ...
      // ofile << "Data Index: " << addr_offset << std::endl;
      // ofile << "Data: " << data << std::endl;

      //// Close handle to file
      // ofile.close();

      save_raw_data(data, addr_offset);
      save_cache_data_load_sparse(addr_offset);
  ...
  else if (current_state == WAITING_FOR_NEXT_STA_ITER && sta_iter_completed)
  {
    ...
    this->str_current_index  = 0;
    this->sta_iter_completed = false;
    if (this->configurationVNs.size() == 1)
    { // If there is only one VN, then maybe foliding has been needed
      this->sta_current_j_metadata = this->sta_last_j_metadata;
      // if(this->configurationVNs[0].getFolding()) {
      //     this->sta_current_j_metadata-=1;
      // }

      if (this->sta_current_j_metadata == this->K)
      { // If this is the end of the cluster, it might start to the next
        this->sta_current_index_metadata += 1;
        this->sta_current_j_metadata = 0;
        std::cout << "STONNE: STA dimensions completed ("
                  << this->sta_current_index_metadata << "/" << this->dim_sta
                  << ")" << std::endl;
      }
    }
    ...
    if (sta_current_index_metadata >= this->dim_sta)
    {
      ...
      sdmemoryStats.sta_sparsity =
        (counter_t)((100 * sta_zeros) / sta_metadata_size);
      sdmemoryStats.str_sparsity =
        (counter_t)((100 * str_zeros) / str_metadata_size);
      this->sdmemoryStats.n_sta_vectors_at_once_avg =
        this->sdmemoryStats.n_sta_vectors_at_once_avg /
        this->sdmemoryStats.n_reconfigurations;
      this->execution_finished = true; // if the last sta cluster has already be
                                       // calculated then finish the sim
...
void SparseSDMemory::send()
{
  ...
  for (int i = 0; i < this->n_read_ports; i++)
  {
    ...
    else if (!this->input_fifos[i]->isEmpty())
    {
...
#ifdef DEBUG_MEM_INPUT
        std::cout << "[MEM_INPUT] Cycle " << local_cycle
                  << ", Sending a WEIGHT through input port " << i << std::endl;
#endif
      }
      else
      {
      ...
      pck_to_send.push_back(pck); // storing into the vector data type structure
                                  // used in class Connection
