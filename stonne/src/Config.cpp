...
void Config::loadFile(std::string config_file)
{
  ...
  // General parameters
  auto print_stats_enabled_conf =
    config->get_as<bool>("print_stats_enabled"); // print_stats_enabled
  ...
  auto n_switches_traversed_by_cycle_conf = config->get_qualified_as<
    unsigned int>(
    "DSNetwork.n_switches_traversed_by_cycle"); // n_switches_traversed_by_cycle
  if (n_switches_traversed_by_cycle_conf)
  {
    this->m_DSNetworkCfg.n_switches_traversed_by_cycle =
      *n_switches_traversed_by_cycle_conf;
  }

  // DSwitch Configuration Parameters
  auto dswitch_latency_conf =
    config->get_qualified_as<unsigned int>("DSwitch.latency"); // Latency
  if (dswitch_latency_conf)
  {
    this->m_DSwitchCfg.latency = *dswitch_latency_conf;
  }

  auto dswitch_input_ports_conf = config->get_qualified_as<unsigned int>(
    "DSwitch.input_ports"); // input_ports
  if (dswitch_input_ports_conf)
  {
    this->m_DSwitchCfg.input_ports = *dswitch_input_ports_conf;
  }

  auto dswitch_output_ports_conf = config->get_qualified_as<unsigned int>(
    "DSwitch.output_ports"); // output_ports
  if (dswitch_output_ports_conf)
  {
    this->m_DSwitchCfg.output_ports = *dswitch_output_ports_conf;
  }

  auto dswitch_port_width_conf =
    config->get_qualified_as<unsigned int>("DSwitch.port_width"); // port_width
  ...
  auto ms_size_conf =
    config->get_qualified_as<unsigned int>("MSNetwork.ms_size");
  if (ms_size_conf)
  {
  ...
  auto ms_rows_conf =
    config->get_qualified_as<unsigned int>("MSNetwork.ms_rows");
  if (ms_rows_conf)
  {
  ...
  auto ms_cols_conf =
    config->get_qualified_as<unsigned int>("MSNetwork.ms_cols");
  if (ms_cols_conf)
  {
    this->m_MSNetworkCfg.ms_cols = *ms_cols_conf;
  }

  auto multiplier_network_type_conf =
    config->get_qualified_as<std::string>("MSNetwork.type"); // Buffers_capacity
  if (multiplier_network_type_conf)
  {
    this->m_MSNetworkCfg.multiplier_network_type =
      get_type_multiplier_network_type(*multiplier_network_type_conf);
  }

  // MSwitch Configuration parameters
  auto mswitch_latency_conf =
    config->get_qualified_as<unsigned int>("MSwitch.latency"); // latency
  if (mswitch_latency_conf)
  {
    this->m_MSwitchCfg.latency = *mswitch_latency_conf;
  }

  auto mswitch_input_ports_conf = config->get_qualified_as<unsigned int>(
    "MSwitch.input_ports"); // input_ports
  if (mswitch_input_ports_conf)
  {
    this->m_MSwitchCfg.input_ports = *mswitch_input_ports_conf;
  }

  auto mswitch_output_ports_conf = config->get_qualified_as<unsigned int>(
    "MSwitch.output_ports"); // output_ports
  if (mswitch_output_ports_conf)
  {
    this->m_MSwitchCfg.output_ports = *mswitch_output_ports_conf;
  }

  auto mswitch_forwarding_ports_conf = config->get_qualified_as<unsigned int>(
    "MSwitch.forwarding_ports"); // forwarding_ports
  if (mswitch_forwarding_ports_conf)
  {
    this->m_MSwitchCfg.forwarding_ports = *mswitch_forwarding_ports_conf;
  }

  auto mswitch_port_width_conf =
    config->get_qualified_as<unsigned int>("MSwitch.port_width"); // port_width
  if (mswitch_port_width_conf)
  {
    this->m_MSwitchCfg.port_width = *mswitch_port_width_conf;
  }

  auto mswitch_buffers_capacity_conf = config->get_qualified_as<unsigned int>(
    "MSwitch.buffers_capacity"); // buffers_capacity
  ...
  auto reduce_network_type_conf = config->get_qualified_as<std::string>(
    "ReduceNetwork.type"); // Buffers_capacity
  if (reduce_network_type_conf)
  {
    this->m_ASNetworkCfg.reduce_network_type =
      get_type_reduce_network_type(*reduce_network_type_conf);
  }

  auto accumulation_buffer_enabled_conf =
    config->get_qualified_as<unsigned int>(
      "ReduceNetwork.accumulation_buffer_enabled");
  if (accumulation_buffer_enabled_conf)
  {
    this->m_ASNetworkCfg.accumulation_buffer_enabled =
      *accumulation_buffer_enabled_conf;
  }

  // ASwitch Configuration Parameters
  auto aswitch_buffers_capacity_conf = config->get_qualified_as<unsigned int>(
    "ASwitch.buffers_capacity"); // Buffers_capacity
  if (aswitch_buffers_capacity_conf)
  {
    this->m_ASwitchCfg.buffers_capacity = *aswitch_buffers_capacity_conf;
  }

  auto aswitch_input_ports_conf = config->get_qualified_as<unsigned int>(
    "ASwitch.input_ports"); // input ports
  if (aswitch_input_ports_conf)
  {
    this->m_ASwitchCfg.input_ports = *aswitch_input_ports_conf;
  }

  auto aswitch_output_ports_conf = config->get_qualified_as<unsigned int>(
    "ASwitch.output_ports"); // output ports
  if (aswitch_output_ports_conf)
  {
    this->m_ASwitchCfg.output_ports = *aswitch_output_ports_conf;
  }

  auto aswitch_forwarding_ports_conf = config->get_qualified_as<unsigned int>(
    "ASwitch.forwarding_ports"); // forwarding ports
  if (aswitch_forwarding_ports_conf)
  {
    this->m_ASwitchCfg.forwarding_ports = *aswitch_forwarding_ports_conf;
  }

  auto aswitch_port_width_conf =
    config->get_qualified_as<unsigned int>("ASwitch.port_width"); // port width
  if (aswitch_port_width_conf)
  {
    this->m_ASwitchCfg.port_width = *aswitch_port_width_conf;
  }

  auto aswitch_latency_conf =
    config->get_qualified_as<unsigned int>("ASwitch.latency"); // latency
  ...
  auto lookuptable_latency_conf =
    config->get_qualified_as<unsigned int>("LookUpTable.latency"); // latency
  if (lookuptable_latency_conf)
  {
    this->m_LookUpTableCfg.latency = *lookuptable_latency_conf;
  }

  auto lookuptable_port_width_conf = config->get_qualified_as<unsigned int>(
    "LookUpTable.port_width"); // port_width
  ...
  auto sdmemory_dn_bw_conf =
    config->get_qualified_as<unsigned int>("SDMemory.dn_bw"); // DN_BW
  if (sdmemory_dn_bw_conf)
  {
    this->m_SDMemoryCfg.n_read_ports = *sdmemory_dn_bw_conf;
  }

  auto sdmemory_rn_bw_conf =
    config->get_qualified_as<unsigned int>("SDMemory.rn_bw"); // RN_BW
  if (sdmemory_rn_bw_conf)
  {
    this->m_SDMemoryCfg.n_write_ports = *sdmemory_rn_bw_conf;
  }

  auto sdmemory_port_width_conf =
    config->get_qualified_as<unsigned int>("SDMemory.port_width");
  if (sdmemory_port_width_conf)
  {
    this->m_SDMemoryCfg.port_width = *sdmemory_port_width_conf;
  }

  auto memory_controller_type_conf =
    config->get_qualified_as<std::string>("SDMemory.controller_type");
  if (memory_controller_type_conf)
  {
    this->m_SDMemoryCfg.mem_controller_type =
      get_type_memory_controller_type(*memory_controller_type_conf);
  }
}
...
void Config::reset()
{
  ...
  // DSNetwork Configuration Parameters
  // ---------------------------------------------------------
  m_DSNetworkCfg.n_switches_traversed_by_cycle =
    23; // From paper. This is not implemented yet anyway.
  ...
  m_DSwitchCfg.latency =
    1; // Actually is less than 1. We do not implement this either
...
{
  return m_SDMemoryCfg.mem_controller_type ==
         SIGMA_SPARSE_GEMM; // If the controller is sparse, then sparsity is
                            // allowed
}
...
void Config::printConfiguration(std::ofstream& out, unsigned int indent)
{
  ...
  out << ind(indent + IND_SIZE)
      << "\"print_stats_enabled\" : " << this->print_stats_enabled << ","
      << std::endl;
...
// -----------------------------------------------------------------------------------------------
void DSNetworkConfig::printConfiguration(std::ofstream& out,
                                         unsigned int indent)
{
  out << ind(indent) << "\"DSNetwork\" : {" << std::endl;
  // Printing DSNetwork configuration
  out << ind(indent + IND_SIZE) << "\"n_switches_traversed_by_cycle\" : "
      << this->n_switches_traversed_by_cycle << std::endl;
  out << ind(indent) << "}";
}
...
void DSwitchConfig::printConfiguration(std::ofstream& out, unsigned int indent)
{
  ...
  out << ind(indent + IND_SIZE) << "\"latency\" : " << this->latency << ","
      << std::endl;
  out << ind(indent + IND_SIZE) << "\"input_ports\" : " << this->input_ports
      << "," << std::endl;
  out << ind(indent + IND_SIZE) << "\"output_ports\" : " << this->output_ports
      << "," << std::endl;
  out << ind(indent + IND_SIZE) << "\"port_width\" : " << this->port_width
      << std::endl;
  out << ind(indent) << "}";
}
...
// -----------------------------------------------------------------------------------------------
void MSNetworkConfig::printConfiguration(std::ofstream& out,
                                         unsigned int indent)
{
  out << ind(indent) << "\"MSNetwork\" : {" << std::endl;
  out << ind(indent + IND_SIZE) << "\"multiplier_network_type\" : "
      << "\""
      << get_string_multiplier_network_type(this->multiplier_network_type)
      << "\""
      << "," << std::endl;
  out << ind(indent + IND_SIZE) << "\"ms_rows\" : " << this->ms_rows << ","
      << std::endl;
  out << ind(indent + IND_SIZE) << "\"ms_columns\" : " << this->ms_cols << ","
      << std::endl;
  out << ind(indent + IND_SIZE) << "\"ms_size\" : " << this->ms_size
      << std::endl;
  out << ind(indent) << "}";
}
...
void MSwitchConfig::printConfiguration(std::ofstream& out, unsigned int indent)
{
  ...
  out << ind(indent + IND_SIZE) << "\"output_ports\" : " << this->output_ports
      << "," << std::endl;
  out << ind(indent + IND_SIZE)
      << "\"forwarding_ports\" : " << this->forwarding_ports << ","
      << std::endl;
  out << ind(indent + IND_SIZE) << "\"port_width\" : " << this->port_width
      << "," << std::endl;
  out << ind(indent + IND_SIZE)
      << "\"buffers_capacity\" : " << this->buffers_capacity << std::endl;
  out << ind(indent) << "}";
}
...
// -----------------------------------------------------------------------------------------------
void ASNetworkConfig::printConfiguration(std::ofstream& out,
                                         unsigned int indent)
{
  out << ind(indent) << "\"ReduceNetwork\" : {" << std::endl;
  out << ind(indent + IND_SIZE) << "\"reduce_network_type\" : "
      << "\"" << get_string_reduce_network_type(this->reduce_network_type)
      << "\""
      << "," << std::endl;
  out << ind(indent + IND_SIZE) << "\"accumulation_buffer_enabled\" : "
      << this->accumulation_buffer_enabled << std::endl;
  out << ind(indent) << "}";
}
...
// -----------------------------------------------------------------------------------------------
void LookUpTableConfig::printConfiguration(std::ofstream& out,
                                           unsigned int indent)
{
...
void SDMemoryConfig::printConfiguration(std::ofstream& out, unsigned int indent)
{
  ...
  out << ind(indent + IND_SIZE) << "\"mem_controller_type\" : "
      << "\"" << get_string_memory_controller_type(this->mem_controller_type)
      << "\""
      << "," << std::endl;
  out << ind(indent + IND_SIZE)
      << "\"write_buffers_capacity\" : " << this->write_buffer_capacity << ","
      << std::endl;
  out << ind(indent + IND_SIZE) << "\"dn_bw\" : " << this->n_read_ports << ","
      << std::endl;
  out << ind(indent + IND_SIZE) << "\"rn_bw\" : " << this->n_write_ports << ","
      << std::endl;
  out << ind(indent + IND_SIZE) << "\"port_width\" : " << this->port_width
      << std::endl;
  out << ind(indent) << "}";
