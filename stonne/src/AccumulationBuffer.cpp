...
// This Constructor creates the reduction tree similar to the one shown in the
// paper
AccumulationBuffer::AccumulationBuffer(id_t id,
                                       std::string name,
                                       Config stonne_cfg,
                                       unsigned int n_accumulators)
  : Unit(id, name)
{
...
void AccumulationBuffer::setMemoryConnections(
  std::vector<Connection*> memoryConnections)
{
  for (int i = 0; i < this->n_accumulators; i++)
  {
...
void AccumulationBuffer::setInputConnections(
...
void AccumulationBuffer::configureSignals(Tile* current_tile,
                                          DNNLayer* dnn_layer,
                                          unsigned int ms_size,
                                          unsigned int n_folding)
{

  this->NPSumsConfiguration(
    n_folding); // All the accumulation buffers have the same folding iteration
                // numbers which means that in this case all the VNs are similar
}

void AccumulationBuffer::resetSignals()
{
  for (std::map<int, Accumulator*>::iterator it = accumulatortable.begin();
       it != accumulatortable.end();
       ++it)
  {
...
void AccumulationBuffer::printConfiguration(std::ofstream& out,
                                            unsigned int indent)
{
}
...
void AccumulationBuffer::printStats(std::ofstream& out, unsigned int indent)
{
  ...
  for (int i = 0; i < this->n_accumulators; i++)
  {
    ...
    if (i == (this->n_accumulators - 1))
    { // If I am in the last accumulator, the comma to separate the accumulators
      // is not added
      out << std::endl; // This is added because the call to acc print do not
                        // show it (to be able to put the comma, if neccesary)
    }
    else
    {
      out << "," << std::endl; // Comma and line break are added to separate
                               // with the next accumulator in the array
    }
  }
...
{
  /*
   The AccumulationBuffer component prints the counters for the next
   subcomponents:
       - Accumualators
       - wires that connect each accumulator with its AdderSwitch

   Note that the wires that connect with memory are not taken into account in
   this component. This is done in the CollectionBus.

  */

  // Printing the input wires

  for (std::map<int, Connection*>::iterator it = inputconnectiontable.begin();
       it != inputconnectiontable.end();
       ++it)
  {
