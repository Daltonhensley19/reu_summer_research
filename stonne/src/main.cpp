...
void configConvParameters(int argc,
                          char* argv[],
                          Config& stonne_cfg,
...
void configSparseGEMMParameters(int argc,
                                char* argv[],
                                Config& stonne_cfg,
                                std::string& layer_name,
                                unsigned int& M,
                                unsigned int& N,
                                unsigned int& K,
                                unsigned int& MK_sparsity,
                                unsigned int& KN_sparsity,
                                Dataflow& dataflow,
                                unsigned int& optimize);

void configDenseGEMMParameters(int argc,
                               char* argv[],
                               Config& stonne_cfg,
                               std::string& layer_name,
                               unsigned int& M,
...
void configSparseDenseParameters(int argc,
                                 char* argv[],
                                 Config& stonne_cfg,
                                 std::string& layer_name,
                                 unsigned int& M,
...
bool runHelpCommand();
// float* generateMatrixDense(unsigned int rows, unsigned int cols, unsigned int
// sparsity);

// void generateSparseDense(unsigned int rows, unsigned int cols, unsigned int
// sparsity);

// unsigned int* generateBitMapFromDense(float* denseMatrix, unsigned int rows,
// unsigned int cols, GENERATION_TYPE gen_type);

float* generateMatrixSparseFromDense(float* denseMatrix,
                                     unsigned int* bitmap,
                                     unsigned int rows,
                                     unsigned int cols,
...
// void generateSparseDense(unsigned int rows, unsigned int cols, unsigned int
// sparsity);
int* generateMinorIDFromDense(float* denseMatrix,
                              unsigned int rows,
                              unsigned int cols,
                              int& nnz,
                              GENERATION_TYPE gen_type);
int* generateMajorPointerFromDense(float* denseMatrix,
                                   unsigned int rows,
                                   unsigned int cols,
                                   GENERATION_TYPE gen_type);
...
void printSparseMatrix(float* sparseMatrix,
                       unsigned int* bitmap,
                       unsigned int rows,
                       unsigned int cols);
...
int main(int argc, char* argv[])
{
  ...
  { // IF there is at least one parameter, -h is checked
    ...
    {
      std::cout << "How to use STONNE User Interface: ./" << argv[0] << " -h"
                << std::endl;
    }
  }
...
bool runConvCommand(int argc, char* argv[])
{
  ...
  configConvParameters(
    ...
    T_N,
    T_X_,
    T_Y_); // Modify stonne_cfg and the variables according to user arguments
  ...
  float* ofmap_cpu =
    new float[ofmap_size]; // Used to store the CPU computed values to compare
                           // with the simulator version
  ...
  Stonne* stonne_instance =
    new Stonne(stonne_cfg); // Creating instance of the simulator
  stonne_instance->loadDNNLayer(CONV,
                                layer_name,
                                R,
                                S,
                                C,
  ...
  stonne_instance->loadTile(
    T_R, T_S, T_C, T_K, T_G, T_N, T_X_, T_Y_); // Loading the tile
  ...
  for (int i = 0; i < ofmap_size; i++)
  {
    ...
    {
      std::cout << "ERROR position " << i
                << ": Value ofmap simulator: " << ofmap[i]
                << ". Value ofmap CPU: " << ofmap_cpu[i] << std::endl;
...
bool runDenseGEMMCommand(int argc, char* argv[])
{
  ...
  configDenseGEMMParameters(
    ...
    T_M,
    T_K,
    T_N); // Modify stonne_cfg and the variables according to user arguments
  ...
  float* output_cpu =
    new float[output_size]; // Used to store the CPU computed values to compare
                            // with the simulator version
  ...
  // computing CPU version based on a Conv parameters mapping. Note a CONV layer
  // might be seen as a GEMM if the mapping is correct. sequential_layer(1, K,
  // 1, M, 1, N, 1, K, 1, KN_matrix, MK_matrix, output_cpu); //Supposes that
  // MK=inputs (M=batch size) and KN=filters (N=number of filters)
  // sequential_layer(1, K, 1, N, 1, 1, M, K, 1, MK_matrix, KN_matrix,
  // output_cpu); //Supposes that MK=inputs (M=batch size) and KN=filters
  // (N=number of filters)
  ...
  sequential_layer(1,
                   ...
                   MK_matrix,
                   KN_matrix,
                   output_cpu); // Supposes that MK=inputs (M=batch size) and
                                // KN=filters (N=number of filters)
  ...
  Stonne* stonne_instance =
    new Stonne(stonne_cfg); // Creating instance of the simulator
  stonne_instance->loadDenseGEMM(layer_name,
                                 N,
                                 K,
                                 M,
                                 MK_matrix,
  ...
  for (int i = 0; i < output_size; i++)
  {
    ...
    {
      std::cout << "ERROR position " << i
                << ": Value ofmap simulator: " << output[i]
                << ". Value ofmap CPU: " << output_cpu[i] << std::endl;
      std::cout << "\033[1;31mT test not passed\033[0m" << std::endl;
      delete[] MK_matrix;
      delete[] KN_matrix;
      delete[] output;
...
bool runSparseGEMMCommand(int argc, char* argv[])
{
  ...
  configSparseGEMMParameters(argc,
                             argv,
                             stonne_cfg,
                             layer_name,
                             M,
  ...
  if (optimize & (dataflow == MK_STA_KN_STR))
  {
    ...
    order_table = calculateOrdering(MK_dense_matrix_no_organized,
                                    M,
                                    K,
                                    GEN_BY_ROWS,
  ...
  {
    order_table = calculateOrdering(KN_dense_matrix_no_organized,
                                    K,
                                    N,
                                    GEN_BY_COLS,
                                    stonne_cfg.m_MSNetworkCfg.ms_size);
  ...
  unsigned int* acc_bitmap =
    new unsigned int[M * N]; // Currently is not generated by the accelerator

  // Generating bitmaps
  unsigned int* MK_bitmap =
    generateBitMapFromDense(MK_dense_matrix, M, K, GEN_BY_ROWS);
  unsigned int* KN_bitmap =
    generateBitMapFromDense(KN_dense_matrix, K, N, GEN_BY_COLS);

  // Generating sparse matrix
  float* MK_sparse_matrix = generateMatrixSparseFromDense(
    MK_dense_matrix, MK_bitmap, M, K, GEN_BY_ROWS);
  float* KN_sparse_matrix = generateMatrixSparseFromDense(
    KN_dense_matrix, KN_bitmap, K, N, GEN_BY_COLS);

  // Running STONNE
  Stonne* stonne_instance =
    new Stonne(stonne_cfg); // Creating instance of the simulator
  stonne_instance->loadGEMM(layer_name,
                            N,
                            K,
                            M,
                            MK_sparse_matrix,
  ...
  cpu_gemm(MK_dense_matrix_no_organized,
           KN_dense_matrix_no_organized,
           cpu_output,
           M,
           N,
           K);
  ...
  */
  ...
  for (int i = 0; i < M; i++)
  {
    for (int j = 0; j < N; j++)
    {
      ...
      if (difference > EPSILON)
      {
        std::cout << "ERROR position (" << i << "," << j
                  << "): Value out simulator: " << acc_output[i * N + j]
                  << ". Value out CPU: " << cpu_output[i * N + j] << std::endl;
        std::cout << "\033[1;31mT test not passed\033[0m" << std::endl;
...
bool runHelpCommand()
{
  std::cout << "Welcome to the STONNE User Interface Version 1.0: "
            << std::endl;
  std::cout << "***************************************************************"
               "********************************************"
            << std::endl;
  std::cout << "***************************************************************"
            ...
            << std::endl
            << std::endl;
  std::cout << "The STONNE user interface allows to execute the STONNE "
               "simulator with any parameter. Currently, STONNE runs"
            << std::endl;
  std::cout << "4 types of operations: Convolution Layers, FC Layers, Dense "
               "GEMMs and Sparse GEMMs. Note that almost any"
            << std::endl;
  std::cout << "kernel can be, in the end, mapped using these operations."
            << std::endl;
  std::cout << std::endl;
  std::cout << "Usage: ./stonne [-h | -CONV | -FC | -SparseGEMM | -DenseGEMM] "
               "[Hardware Parameters] [Dimension and tile Parameters]"
            << std::endl;
  std::cout << std::endl;
  std::cout << "[Hardware parameters]" << std::endl;
  std::cout << "-num_ms=[x]: Number of multiplier switches (must be power of 2)"
            << std::endl;
  std::cout
    << "-dn_bw=[x]: Number of read ports in the SDMemory (must be power of 2)"
    << std::endl;
  std::cout
    << "-rn_bw=[x]: Number of write ports in the SDMemory (must be power of 2)"
    << std::endl;
  std::cout << "-rn_type=[0=ASNETWORK, 1=FENETWORK]: type of the ReduceNetwork "
               "to be used (Not supported for SparseGEMM)"
            << std::endl;
  std::cout << "-accumulation_buffer=[0,1]: enables the accumulation buffer"
            << std::endl;
  std::cout
  ...
  std::cout << "The dimensions of the kernel depends on the type of the "
               "operation that is going to be run. Next, we describe"
            << std::endl;
  std::cout << "described the dimensions according to each supported operation."
            << std::endl;
  std::cout << std::endl;
  std::cout << "[CONV]" << std::endl;
  std::cout << "-layer_name: Name of the layer to run. The output statistic "
               "file will be named accordingly"
            << std::endl;
  ...
  std::cout
    << "-T_N: Number of inputs mapped at a time (Only 1 is supported so far)"
    << std::endl;
  ...
  std::cout << "1. Number of Virtual Neurons mapped (Num_VNs) will be "
               "T_K*T_G*T_N*T_X_*T_T_"
            << std::endl;
  std::cout << "2. The minimum number of MSwitches needed will be at least "
               "VN_Size*Num_VNs"
            << std::endl;
  std::cout << "3. Note in case of folding (iteration over the same VN) is "
               "enabled, and the accumulation buffer disabled"
            << std::endl;
  std::cout << "1 extra MSwitch per VN will be needed to manage the psum."
            << std::endl;
  std::cout << "In this case, the minimum number of MSwitches needed will be "
               "at least (VN_Size+1)*Num_VNs"
            << std::endl;
  std::cout << "Folding (iteration over the same virtual neuron) will be "
               "enabled if (R/T_S)*(S/T_S)*(C/T_C) > 1"
            << std::endl;
  std::cout << std::endl;
  std::cout << "[FC]" << std::endl;
  std::cout << "-layer_name: Name of the layer to run. The output statistic "
               "file will be called by this name"
            << std::endl;
  ...
  std::cout << "** Please take into consideration that: **" << std::endl;
  std::cout << "1. Number of Virtual Neurons mapped (Num_VNs) will be T_N*T_M"
            << std::endl;
  std::cout
    << "2. The minimum number of MSwitches needed will be at least T_K*Num_VNs."
    << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "-K=Number of columns MK and rows KN matrix (cluster size)"
            << std::endl;
  ...
  std::cout << "-K=Number of columns MK and rows KN matrix (cluster size)"
            << std::endl;
  std::cout << "-MK_sparsity=Percentage of sparsity MK matrix (0-100)"
            << std::endl;
  std::cout << "-KN_sparsity=Percentahe of sparsity KN matrix (0-100)"
            << std::endl;
  std::cout << "-dataflow=MK_STA_KN_STR or MK_STR_KN_STA" << std::endl;
  std::cout << "-optimize=[0,1]: apply compiler-based optimizations"
            << std::endl;
...
// This function modifies the default values of the parameters according to user
// arguments.
void configConvParameters(int argc,
                          char* argv[],
                          Config& stonne_cfg,
                          ...
                          unsigned int& T_Y_)
{
  ...
  { // 0 is the name of the program and 1 is the execution command type
    ...
    if (arg.npos != pos)
    {
      ...
      unsigned int value;
      if ((name != "-layer_name") && (name != "-rn_type"))
      { // string parameters
      ...
      else if (name == "-rn_type")
      {
        ...
        stonne_cfg.m_ASNetworkCfg.reduce_network_type =
          get_type_reduce_network_type(value_str);
      }
      ...
      {
        std::cout << "Error: parameter " << name << " does not exist"
                  << std::endl;
        exit(1);
      }
    }
  ...
  { // 0 is the name of the program and 1 is the execution command type
    ...
    if (arg.npos != pos)
    {
      ...
      else if (name == "-mn_type")
      {
        ...
        stonne_cfg.m_MSNetworkCfg.multiplier_network_type =
          get_type_multiplier_network_type(value_str);
      }

      else if (name == "-mem_ctrl")
      {
        std::cout << "Changing mem_ctrl to " << value_str << std::endl;
        stonne_cfg.m_SDMemoryCfg.mem_controller_type =
          get_type_memory_controller_type(value_str);
      }
                                ...
                                unsigned int& optimize)
{
  ...
  { // 0 is the name of the program and 1 is the execution command type
    ...
    if (arg.npos != pos)
    {
      ...
      string name      = arg.substr(0, pos);
      unsigned int value;
      if ((name != "-layer_name") && (name != "-dataflow"))
      { // string parameters
        value = stoi(value_str);
      }
      // Checking parameter name
