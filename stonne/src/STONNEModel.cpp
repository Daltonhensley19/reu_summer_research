...
Stonne::Stonne(Config stonne_cfg)
{
  ...
  switch (stonne_cfg.m_ASNetworkCfg.reduce_network_type)
  {
    case ASNETWORK:
      this->asnet =
        new ASNetwork(3, "ASNetwork", stonne_cfg, outputASConnection);
      break;
    case FENETWORK:
      this->asnet =
        new FENetwork(3, "FENetwork", stonne_cfg, outputASConnection);
      break;
    case TEMPORALRN:
      this->asnet =
  ...
  this->lt            = new LookupTable(
    5, "LookUpTable", stonne_cfg, outputASConnection, outputLTConnection);
  ...
  switch (stonne_cfg.m_SDMemoryCfg.mem_controller_type)
  {
    case SIGMA_SPARSE_GEMM:
      this->mem = new SparseSDMemory(
        0, "SparseSDMemory", stonne_cfg, this->outputLTConnection);
      break;
    case MAERI_DENSE_WORKLOAD:
      this->mem =
        new SDMemory(0, "SDMemory", stonne_cfg, this->outputLTConnection);
      break;
    case TPU_OS_DENSE:
      this->mem = new OSMeshSDMemory(
        0, "OSMeshSDMemory", stonne_cfg, this->outputLTConnection);
      break;
    default:
      assert(false);
  }
  // Adding to the memory controller the asnet and msnet to reconfigure them if
  // needed
...
void Stonne::loadDNNLayer(Layer_t layer_type,
                          ...
                          Dataflow dataflow)
{
  ...
  {
    // assert((R==1) && (C==1) && (G==1) && (Y==S) && (X==1)); //Ensure the
    // mapping is correct
  }
  this->dnn_layer =
    new DNNLayer(layer_type, layer_name, R, S, C, K, G, N, X, Y, strides);
  this->layer_loaded = true;
  this->mem->setLayer(
    this->dnn_layer, input_address, filter_address, output_address, dataflow);
}

void Stonne::loadCONVLayer(std::string layer_name,
                           unsigned int R,
                           unsigned int S,
                           unsigned int C,
                           unsigned int K,
...
void Stonne::loadFCLayer(std::string layer_name,
                         unsigned int N,
                         unsigned int S,
                         unsigned int K,
                         address_t input_address,
                         ...
                         address_t output_address)
{
  // loadDNNLayer(FC, layer_name, 1, S, 1, K, 1, N, 1, S, 1, input_address,
  // filter_address, output_address, CNN_DATAFLOW);
  loadDNNLayer(FC,
               layer_name,
               1,
               S,
...
void Stonne::loadGEMM(std::string layer_name,
                      ...
                      address_t MK_matrix,
                      address_t KN_matrix,
                      metadata_address_t MK_metadata,
                      metadata_address_t KN_metadata,
                      address_t output_matrix,
...
void Stonne::loadDenseGEMM(std::string layer_name,
                           unsigned int N,
                           unsigned int K,
                           unsigned int M,
                           address_t MK_matrix,
...
void Stonne::loadSparseDense(std::string layer_name,
                             unsigned int N,
                             unsigned int K,
                             unsigned int M,
                             address_t MK_matrix,
                             address_t KN_matrix,
                             metadata_address_t MK_metadata_id,
                             metadata_address_t MK_metadata_pointer,
                             address_t output_matrix,
                             unsigned int T_N,
                             unsigned int T_K)
{
  ...
  std::cout << "Loading a Sparse multiplied by dense GEMM into STONNE"
            << std::endl;
...
void Stonne::loadGEMMTile(unsigned int T_N, unsigned int T_K, unsigned int T_M)
{
  ...
  assert(this->layer_loaded &&
         (this->dnn_layer->get_layer_type() ==
          GEMM)); // Force to have the right layer with the GEMM parameters)
  std::cout << "Loading a GEMM tile" << std::endl;
}

// To dense CNNs and GEMMs
void Stonne::loadTile(unsigned int T_R,
                      unsigned int T_S,
                      unsigned int T_C,
                      unsigned int T_K,
                      unsigned int T_G,
                      ...
                      unsigned int T_Y_)
{
  ...
  {
    assert(this->ms_size >= (T_R * T_S * T_C * T_K * T_G * T_N * T_X_ *
                             T_Y_)); // There are enough mswitches
  }
  else
  {
    assert((this->stonne_cfg.m_MSNetworkCfg.ms_rows *
            this->stonne_cfg.m_MSNetworkCfg.ms_cols) >=
           (T_R * T_S * T_C * T_K * T_G * T_N * T_X_ * T_Y_));
  }
  // Checking if the dimensions fit the DNN layer. i.e., the tile is able to
  // calculate the whole layer.
  std::cout << "Loading Tile: <T_R=" << T_R << ", T_S=" << T_S
            << ", T_C=" << T_C << ", T_K=" << T_K << ", T_G=" << T_G
            << ", T_N=" << T_N << ", T_X'=" << T_X_ << ", T_Y'=" << T_Y_ << ">"
            << std::endl;

  // Remove these lines if we want the architeture to compute the layer even if
  // the tile does not fit.
  //  This will mean that some row, columns or output channels would remain
  //  without calculating.
  if (stonne_cfg.m_SDMemoryCfg.mem_controller_type == MAERI_DENSE_WORKLOAD)
  { // Just for this maeri controller
    ...
    assert((this->dnn_layer->get_X_() % T_X_) ==
           0); // T_X_ must be multiple of X_
    assert((this->dnn_layer->get_Y_() % T_Y_) ==
           0); // T_Y_ must be multiple of Y_
  }

  // End check
  unsigned int n_folding = (this->dnn_layer->get_R() / T_R) *
                           (this->dnn_layer->get_S() / T_S) *
                           (this->dnn_layer->get_C() / T_C);
  bool folding_enabled =
    false; // Condition to use extra multiplier. Note that if folding is enabled
           // but some type of accumulation buffer is needed this is false as no
           // fw ms is needed.
  if ((n_folding > 1) &&
      (this->stonne_cfg.m_ASNetworkCfg.accumulation_buffer_enabled == 0) &&
      (this->stonne_cfg.m_ASNetworkCfg.reduce_network_type != FENETWORK))
  { // If there is folding and the RN is not able to acumulate itself, we have
    // to use an extra MS to accumulate
    folding_enabled = true;
    // When there is folding we leave one MS free per VN aiming at suming the
    // psums. In next line we check if there are
    //  enough mswitches in the array to support the folding.
    assert(this->ms_size >=
           ((T_R * T_S * T_C * T_K * T_G * T_N * T_X_ * T_Y_) +
            (T_K * T_G * T_N * T_X_ * T_Y_))); // We sum one mswitch per VN
  }
  this->current_tile =
    new Tile(T_R, T_S, T_C, T_K, T_G, T_N, T_X_, T_Y_, folding_enabled);

  // Generating the signals for the reduceNetwork and configuring it. The
  // asnet->configureSignals will call its corresponding compiler to generate
  // the signals and allocate all of them
  if (this->stonne_cfg.m_MSNetworkCfg.multiplier_network_type != OS_MESH)
  { // IN TPU the configuration is done in the mem controller
    this->asnet->configureSignals(
      this->current_tile,
      this->dnn_layer,
      this->ms_size,
      n_folding); // Calling the ART to configure the signals with them
                  // previously generated
    // Getting MN signals
    this->msnet->configureSignals(
      this->current_tile, this->dnn_layer, this->ms_size, n_folding);
  }
  // Setting the signals to the corresponding networks

  // If stride > 1 then all the signals of ms_fwreceive_enabled and
  // ms_fwsend_enabled must be disabled since no reuse between MSwitches can be
  // done. In order to not to incorporate stride as a tile parameter, we leave
  // the class Tile not aware of the stride. Then, if stride exists, here the
  // possible enabled signals (since tile does not know about tile) are
  // disabled.
...
void Stonne::loadFCTile(unsigned int T_S, unsigned int T_N, unsigned int T_K)
{
  ...
  assert(this->layer_loaded &&
         (this->dnn_layer->get_layer_type() ==
          FC)); // Force to have the right layer with the FC parameters)
...
void Stonne::cycle()
{
  ...
  while (!execution_finished)
  {
    ...
    this->asnet->cycle();
    this->lt->cycle();
    //        this->collectionBus->cycle(); //This order since these are
    //        connections that have to be seen in next cycle
    end = std::chrono::steady_clock::now();
    this->time_as +=
      std::chrono::duration_cast<std::chrono::microseconds>(end - start)
        .count();
  ...
  if (this->stonne_cfg.print_stats_enabled)
  { // If sats printing is enable
    this->printStats();
    this->printEnergy();
  }
...
void Stonne::printStats()
{
  ...
  out.open(output_directory_str + "output_stats_layer_" +
           this->dnn_layer->get_name() + "_architecture_MSes_" +
           std::to_string(num_ms) + "_dnbw_" + std::to_string(dn_bw) + "_" +
           "rn_bw_" + std::to_string(rn_bw) + "timestamp_" +
  ...
  // Printing ASNetwork configuration parameters (i.e., ASwitches configuration
  // for these VNs, flags, etc)
...
void Stonne::printEnergy()
{
  ...
  this->dsnet->printEnergy(out, indent); // DSNetworkTop //DSNetworks
                                         // //DSwitches
 ...
 */
  ...
  // Unicast test
  /*
  DataPackage* data_to_send = new DataPackage(32, 500, IACTIVATION, 0, UNICAST,
  6); std::vector<DataPackage*> vector_to_send;
  vector_to_send.push_back(data_to_send);
  this->inputConnection->send(vector_to_send);
  */
