...
Bus::Bus(id_t id, std::string name, Config stonne_cfg)
  : Unit(id, name)
{
  this->n_bus_lines = stonne_cfg.m_SDMemoryCfg.n_write_ports;
  this->input_ports_bus_line =
    (stonne_cfg.m_MSNetworkCfg.ms_size / this->n_bus_lines) + 1;
  ...
  for (int i = 0; i < n_bus_lines; i++)
  {
    ...
    CollectionBusLine* busline =
      new CollectionBusLine(i,
                            name,
                            i,
                            this->input_ports_bus_line,
...
std::vector<std::vector<Connection*>> Bus::getInputConnections()
{
  ...
  for (int i = 0; i < n_bus_lines; i++)
  { // For each bus line we get all their connections and put them into the
    // structure
    CollectionBusLine* collection_bus = collection_bus_lines[i];
    std::vector<Connection*> connections_collection_bus =
      collection_bus->getInputConnections();
    connections.push_back(
      connections_collection_bus); // Setting the connections for that busID
  }
  return connections;
}

Connection* Bus::getInputConnectionFromBusLine(unsigned int busID,
...
void Bus::printStats(std::ofstream& out, unsigned int indent)
{
  ...
  out << ind(indent + IND_SIZE) << "\"CollectionBusLineStats\" : ["
      << std::endl; // One entry per BusLine
  for (int i = 0; i < this->collection_bus_lines.size(); i++)
  {
    collection_bus_lines[i]->printStats(out, indent + IND_SIZE + IND_SIZE);
    if (i == (this->collection_bus_lines.size() - 1))
    { // If I am in the last BusLine, the comma to separate the BusLines is not
      // added
