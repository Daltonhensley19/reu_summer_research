...
float* Transform_Ifmap_Memory(const float* bottom_data,
                              const int C,
                              const int X,
                              const int Y,
                              const int pad_x,
...
float* Transform_Filters_Memory(const float* weights,
                                const int K,
                                const int G,
                                const int C,
                                const int R,
...
void Transform_Ofmap_Memory(const float* ofmap_data,
                            float* top_data,
                            const int K,
                            const int X_,
                            const int Y_);
    ...
    for (int y = 0; y < output_y; y++)
    {
      for (int x = 0; x < output_x; x++)
      {
        ...
        top_data[i * size_channel + y * output_x + x] =
          ofmap_data[(n_channels * (y * output_x + x)) + i]; // Filling top_data
      }
    }
  }
}

void simulateDenseConvForward(std::string layer_name,
                              float* input,
                              float* weight,
                              float* output,
                              int R,
...
*/
  ...
  stonne_instance->loadDNNLayer(CONV,
                                ...
                                Y + 2 * pad_y,
                                strides,
                                (address_t)ifmap_to_send,
                                (address_t)filters_to_send,
                                (address_t)ofmap_raw,
                                CNN_DATAFLOW);
  ...
  // sequential_layer(R, S, C, K, G, N, X, Y, strides, (address_t)ifmap_to_send,
  // (address_t)filters_to_send, (address_t)ofmap_raw);
...
void simulateSparseGemmForward(std::string layer_name,
                               float* KN_matrix_raw,
                               float* MK_matrix_raw,
                               float* output_raw,
                               int N,
                               ...
                               Dataflow dataflow)
{
  ...
  for (int n = 0; n < N; n++)
  {
    ...
    { // Every group is calculated with a different GEMM
      ...
      float* MK_sparse_matrix = generateMatrixSparseFromDense(MK_dense_matrix,
                                                              MK_bitmap,
                                                              gemm_M_grouped,
                                                              gemm_K_grouped,
                                                              GEN_BY_ROWS);
      float* KN_sparse_matrix = generateMatrixSparseFromDense(KN_dense_matrix,
                                                              KN_bitmap,
                                                              gemm_K_grouped,
                                                              gemm_N_grouped,
                                                              GEN_BY_COLS);
      ...
      stonne_instance->loadGEMM(layer_name_group,
                                gemm_N_grouped,
                                gemm_K_grouped,
                                gemm_M_grouped,
                                MK_sparse_matrix,
...
void simulateSparseGemmWithBitmapsForward(std::string layer_name,
                                          float* KN_matrix_raw,
                                          float* MK_matrix_raw,
                                          float* output_raw,
                                          int N,
...
void simulateDenseGemmForward(std::string layer_name,
                              float* KN_matrix_raw,
                              float* MK_matrix_raw,
                              float* output_raw,
                              int N,
                              ...
                              Config stonne_cfg)
{
  ...
  for (int n = 0; n < N; n++)
  {
    ...
    { // Every group is calculated with a different GEMM
      ...
      stonne_instance->loadFCLayer(layer_name_group,
                                   gemm_M_grouped,
                                   gemm_K_grouped,
                                   gemm_N_grouped,
                                   (address_t)pointer_MK_dense_matrix,
                                   (address_t)pointer_KN_dense_matrix,
                                   (address_t)acc_output);
