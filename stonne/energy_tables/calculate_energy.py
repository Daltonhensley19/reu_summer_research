...
def read_arguments():
    ...
    if ((table_file == "") or (counter_file == "") or (mode == "")):
        print("Error parsing input parameters")
        print("Usage: ./" + str(
            sys.argv[0] +
            " [-v] -table_file=<Energy numbers file> -counter_file=<Runtime counters file> -[out_file=<output file>]"
        ))
        exit(2)
    ...
    return verbose, table_file, counter_file, out_file, mode
...
#  @return energy_table s a dictionary whose keys are the components and every key corresponds with another dictionary
...
def getEnergyTable(table_file, out_file):
    ...
    for line in f:
        ...
        if (len(line_splited) >= 1):
            ...
            energy_table[component] = {
            }  # Creating the dictionary for this component
            for op in line_splited[1:]:
                op_splited = op.split('=')
                if (len(op_splited) != 2):
                    print('Error parsing line ' + str(line_number) + ": " +
                ...
                energy_table[component][
                    operation] = energy  #Inserting key=operation value=energy
...
def calculateEnergy(energy_table, counter_file, out_file, verbose, mode):
    cf = open(counter_file, "r")  # We are sure it exists
    out_file = open(out_file,
                    "a")  # We are sure it was created in getEnergyTable
    line_number = 0
    ...
    dynamic_energy_component = {
    }  # Dictionary with the dynamic energy of every high level compo
    static_energy_component = {
    }  # Dictionary with the static energy of every high level compo
    area_component = {
    }  # Dictionary with the area of every high level cmponent
    ...
    for line_counter in cf:
        ...
        if (line_counter_clean != ""):  # Ignore new and void lines
            if ((line_counter_clean[0] == "[") and
                (line_counter_clean[len(line_counter_clean) - 1] == "]")):
                if (
                        verbose
                ):  # If verbose we are going to print every component of every high level component
                    out_file.write(line_counter)
                #Updating current component
                current_component = line_counter_clean[1:len(line_counter_clean
                                                             ) - 1]
                ...
                if ("STATIC" in energy_table[component]):
                    if mode == "PDP":
                        static_energy = float(
                            ...
                            float(energy_table[component]["STATIC"]))
                    elif mode == "EDP":
                        static_energy = float(
                            float(number_of_cycles * number_of_cycles) *
                            float(energy_table[component]["STATIC"]))
                else:
                ...
                for op in line_counter_splited[1:]:
                    ...
                    op_splited = op.split(
                        '=')  # Separating the operation and the counter
                    if (len(op_splited) != 2):
                        print('Error to parse line ' + str(line_number) +
                              ": " + op + " not recognized")
                        exit(3)
                    ...
                    if mode == "PDP":
                        dynamic_energy = float(
                            float(counter) *
                            float(energy_table[component][operation]))
                    elif mode == "EDP":
                        dynamic_energy = float(
                    ...
                    #accumulate
                    dynamic_energy_component[
                        current_component] += dynamic_energy
...
def main():
    ...
    verbose, table_file, counter_file, out_file, mode = read_arguments()
    ...
    calculateEnergy(energy_table, counter_file, out_file, verbose, mode)
    print(verbose)
    print(table_file)
    print(counter_file)
    print(out_file)
