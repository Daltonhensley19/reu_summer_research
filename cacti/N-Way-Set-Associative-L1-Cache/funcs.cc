...
void cacheMemmoryAllocation()
{
  ...
  chacheAllocation = (CacheSet*)malloc(((totalCacheSets) * sizeof(CacheSet)));
  ...
  try
  {
    for (; k < totalCacheSets; k++)
      new (chacheAllocation + k) CacheSet(blockSize,
                                          tagbits_tot,
                                          indexbits_tot,
                                          offsetbits_tot,
                                          associativity,
  ...
  {
    chacheAllocationIns =
      (CacheSet*)malloc(((totalCacheSets) * sizeof(CacheSet)));
    ...
    {
      for (; k < totalCacheSets; k++)
        new (chacheAllocationIns + k) CacheSet(blockSize,
                                               tagbits_tot,
                                               indexbits_tot,
                                               offsetbits_tot,
                                               associativity,
...
std::string wordAddress(int i)
{
  ...
  stream << "" << std::setfill('0') << std::setw(INT_SIZE * 2) << std::hex << i;
...
void displayStatistics()
{
  ...
  std::cout << "";
  std::cout << "Total Cache Accesses = " << totalAccess << std::endl;
  std::cout << "Cache Reads: " << total_Read << std::endl;
  std::cout << "Cache Writes: " << total_Write << std::endl;
  std::cout << "Cache_size: " << chacheCapacity << std::endl;;
  ...
  if (!split)
  {
    printf("Misses: Total %d DataReads %d DataWrites %d\n",
           ...
           missRead,
           missWrite);
    printf("Miss Rate: Totalrio %f Readrio %f Writerio %f\n",
...
void displayMainMemory()
{
  ...
  std::cout << std::endl << "MAIN MEMORY:" << std::endl << "Address      Words";
       ...
       i++)
  {
    if (i % 8 == 0)
      std::cout << std::endl
                << std::setfill('0') << std::setw(8) << std::hex << i * INT_SIZE
                << "   ";
    std::cout << "  " << std::setfill('0') << std::setw(8) << ramAllocation[i];
  }
  std::cout << std::endl;
}
...
void parseMemoryAddress(char* bformatted, char* tag, char* index, char* offset)
{
  ...
  for (i = indexbits_tot + tagbits_tot + 2;
       i < offsetbits_tot + indexbits_tot + tagbits_tot + 2;
       i++)
  {
                 ...
                 bool& split)
{
  ...
  while (1)
  {
    ...
    int option_index                    = 0;
    static struct option long_options[] = {{"wbwa", 0, 0, 0},
                                           {"wtwn", 0, 0, 0},
                                           {"wbwn", 0, 0, 0},
                                           {"wtwa", 0, 0, 0},
                                           {0, 0, 0, 0}};
    ...
    c =
      getopt_long(argc, argv, "c:a:b:s::0:1:2:3:", long_options, &option_index);
    if (c == -1)
      break;
  ...
  {
    std::cout << "\nPossible Block capacities are 4, 8, 16, 32, 64, 128, 256, "
                 "or 512.\n";
    errflg = true;
  }
  ...
  {
    fprintf(stderr,
            "usage: %s -c<capacity> -b<wordsize> -a<associativity> -s<for "
            "Split> < inputTrace.trace > outputFile.txt \nWrite policies are "
            "as follows: --wbwa/--wbwn/--wtwa/--wtwn",
            argv[0]);
    return false;
  }
