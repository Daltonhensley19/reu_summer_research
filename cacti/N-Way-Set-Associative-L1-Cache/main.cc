...
void loadBlock(CacheSet* cacheset,
               int setNumber,
               int blockNumber,
               cacheline line,
               int ramaddress);
void writeBlock(CacheSet* cacheset,
                int setNumber,
                int blockNumber,
                cacheline line,
                int ramaddress);
void updateDataCache(CacheSet* cacheset,
                     int setNumber,
                     int freeBlockNo,
                     int offset,
                     unsigned int data,
    ...
    while (!feof(stdin))
    {
      ...
      if (read_write == MEM_WRITE)
      {
        ...
        int blockNo = findBlockNumber(
          chacheAllocation, binaryToInteger(temp.index), temp.tag);
        if (blockNo == -1)
        {
          blockNo =
            blockToWrite(chacheAllocation, binaryToInteger(temp.index), temp);
          if (chacheAllocation[binaryToInteger(temp.index)]
                .set[blockNo]
                .dirty == 1)
          {
      ...
      else if (read_write == MEM_READ)
      {
        ...
        total_Read++;
        // std::cout << std::hex << "memory[" << address << "] = " <<
        // readDataCache(binaryToInteger(temp.index),binaryToInteger(temp.current_offset))
        //	<< std::endl;
      }
      else if (split && read_write == INS_READ)
      {
        ...
        {
          int blockNo = findBlockNumber(
            chacheAllocationIns, binaryToInteger(temp.index), temp.tag);
          if (blockNo == -1)
          {
            blockNo = blockToWrite(
              chacheAllocationIns, binaryToInteger(temp.index), temp);
